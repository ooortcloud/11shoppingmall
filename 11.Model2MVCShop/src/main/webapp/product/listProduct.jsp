							...
							select: function(event, ui) {  // callback :: 사용자가 추천 검색어를 클릭했을 때
								...
								$.ajax( { 
									...
									success: function(responseBody, httpStatus) {
										...
										/// page navigator
										temp="";
										// const pageNavigator = $('nav > ul');  // 1촌으로 제한하지 않으면 toolbar까지 바꿔버림 << pager 배열이 다닥다닥 붙어버려서 못 씀
										const pageNagivator = $('#page-navigator');  // 아 이렇게 해도 다닥다닥 붙어버린다...
										...
										if(responseBody.resultPage.endUnitPage >= responseBody.resultPage.maxPage) {
											...
											temp += '<li>'
															+ '<a href="javascript:fncGetProductList('+(responseBody.resultPage.endUnitPage + 1)+')">'
